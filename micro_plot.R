micro_plot<-function(gene,length,seq,filepath,pos1,pos2=NULL){
  ...
  #   annotate("text",x=pos1,y=1.12,label="g1",color="red",size=5)
  ...
  if(!is.null(pos2)){
    # img2<-"cut2.png"
    f4<-data.frame(x=pos2,y=1.09)
    p<-p+geom_image(data = f4,aes(x = x, y = y),image=img)+
      annotate("text",x=pos2,y=1.17,label="g2",color="black",size=5)
  }
...
# 
# sample<-read.csv("C://Users//41518//Desktop//Î¢ÉúÎï//micro.csv")
# sample<-sample[which(sample$species=="K-12 substr. MG1655"),]
# 
# sam<-sample[which(sample$gene=="yabI"),]
# 
# gene<-sam$gene
