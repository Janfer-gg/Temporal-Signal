...
makeAlignedArray = js2py.eval_js('''
...
display3EndDimer = js2py.eval_js('''
function display3EndDimer(theAlignedArray,seq,seq_list,compare_list)
{
	var d=seq.length;
	var returnString="";
	var s1="";
	...
	for(var n=0; n<theAlignedArray.length-1; n++) //the last element in array is junck
	{
		...
		if(theAlignedArray[n][2]==d-1)		//end position of match in original seq
		{
			...
			for(i=0; i<d; i++)
			{
				...
				{
					s1+="<FONT COLOR='red'>"+seq_list[i]+"</FONT>";
			...
			for(k=0; k<d; k++)
			{
				...
				{
					s2+="<FONT COLOR='red'>"+compare_list[k]+"</FONT>";
	...
	return returnString+="2222222222";
}

''')

displayAllDimers = js2py.eval_js('''
...
*/
	...
	if(theAlignedArray.length>1)
	{
		...
		for(n=0; n<theAlignedArray.length; n++)
		{
			...
			s1+=theSequence.substring(0,theAlignedArray[n][0])+
				theSequence.substring(theAlignedArray[n][0],theAlignedArray[n][2]+1)+
			...
			s2+=reversedSeq.substring(0,theAlignedArray[n][1])+
				reversedSeq.substring(theAlignedArray[n][1],theAlignedArray[n][3]+1)+
	...
	returnString+="</PRE>333333";
	return returnString;	
}
''')


def get_AlignedArray(matrix):
    lenth = len(matrix)
    minLen, maxMisMatch = 5, 1

    theResults = []
    for i, item in enumerate(matrix):
        for j, mat in enumerate(item):
            if mat == 1:
                mismatches = 0
                hasMatch = 1
                lastMatch = 1
                ...
                for k in range(1, maxInc):
                    ...
                    lastMatch = hasMatch
                if k - mismatches >= minLen:
                    theResults.append([i, j, i + k - 1, j + k - 1, mismatches])
    return theResults
...
def Compare(seq):
    ...
    reserve = map(lambda x: rese_dict[x], reserve)
    return reserve
...
def AllNoneSeq(seq):
    ...
    compare = ''.join(list(Compare(seq_list)))
    compare_list = [c for c in compare]
...
function isBaseEqual( c1, c2)
{
	...
	if(broadMatch){
		...
		var equA="AMRWVHD"; // lack of 'M' caught by Paul Wayper. Thanks Paul!
		var equT="TWYKHDB";
		var equG="GRSKVDB";
		var equC="CMSYVHB"; // lack of 'M' caught by Paul Wayper. Thanks Paul!
		...
		if(c1 == 'M' ) { if(equA.indexOf(c2)>=0 || equC.indexOf(c2)>=0) {return true;} return false;}
    ...
    '''
    matrix = execjs.compile(fillMatchmatrix).call('makeMatrix', lenth)

    matrix = fillMatchMatrix(seq, compare, matrix)
    k = 0

    AlignedArray = get_AlignedArray(matrix)
    oligo_js = '''
...
function calcPrimer(form) {
    ...
    if (theOligo.seqArray)
        delete theOligo.seqArray;
    if (theOligo.revSeqArray)
        delete theOligo.revSeqArray;
    if (theComplement.seqArray)
        delete theComplement.seqArray;
    if (theComplement.revSeqArray)
        delete theComplement.revSeqArray;
    ...
    minAlignLen = parseInt(form.selfComp.options[form.selfComp.selectedIndex].value);
    //for 3' complementarity
    minHairpinLen = parseInt(form.hairpin.options[form.hairpin.selectedIndex].value);
    //for hairpin
...
function calculateMatrices() {
    ...
    if (theOligo.Sequence.length != theComplement.Sequence.length) {
        return "Error! Primer and its complement are different lengths!<br>";
    }
    //setup d*d matrix
        ...
        for (seqPos = 0; seqPos < theFullSequence.length - maxSeqLength; seqPos++) {
            ...
            if (theResult[0] > -1) {
                ...
                if (seqPos + minHairpinLength >= maxSeqLength) {
                    ...
                    break;
                    // we have moved far enough on the primer to guarentee we have everything  -further would give us the reverse match
                }
            } else {
                ...
                // move compPos forward to stop identical checks if long match was found!
                break;
                //not found in the rest of the sequence!
            }
        }
    }
    return theResults;
...
*/
    ...
    for (i = 0; i < matLength; i++) {
        for (j = 0; j < matLength; j++) {
            if (mat[i][j] == 1) {
                //potential start of an alignment
                mismatches = 0;
                hasMatch = 1;
                lastMatch = 1;
                maxInc = matLength - (i <= j ? j : i);
...
function arrayOrder(a, b) {
    return (((a[2] - a[0]) < (b[2] - b[0])) ? 1 : (((a[2] - a[0]) > (b[2] - b[0])) ? -1 : (a[0] - a[1]) - (b[0] - b[1])));
}
//size plus position
...
function isBaseEqual(c1, c2) {
    ...
    if (broadMatch) {
        ...
        if (c1 == 'A') {
            if (equA.indexOf(c2) >= 0) {
                return true;
            }
            return false;
        }
        if (c1 == 'T') {
            if (equT.indexOf(c2) >= 0) {
                return true;
            }
            return false;
        }
        if (c1 == 'G') {
            if (equG.indexOf(c2) >= 0) {
                return true;
            }
            return false;
        }
        if (c1 == 'C') {
            if (equC.indexOf(c2) >= 0) {
                return true;
            }
            return false;
        }
        if (c1 == 'M') {
            if (equA.indexOf(c2) >= 0 || equC.indexOf(c2) >= 0) {
                return true;
            }
            return false;
        }
        if (c1 == 'R') {
            if (equA.indexOf(c2) >= 0 || equG.indexOf(c2) >= 0) {
                return true;
            }
            return false;
        }
        if (c1 == 'W') {
            if (equA.indexOf(c2) >= 0 || equT.indexOf(c2) >= 0) {
                return true;
            }
            return false;
        }
        if (c1 == 'S') {
            if (equG.indexOf(c2) >= 0 || equC.indexOf(c2) >= 0) {
                return true;
            }
            return false;
        }
        if (c1 == 'Y') {
            if (equT.indexOf(c2) >= 0 || equC.indexOf(c2) >= 0) {
                return true;
            }
            return false;
        }
        if (c1 == 'K') {
            if (equT.indexOf(c2) >= 0 || equG.indexOf(c2) >= 0) {
                return true;
            }
            return false;
        }
        if (c1 == 'V') {
            if (equA.indexOf(c2) >= 0 || equG.indexOf(c2) > 0 || equC.indexOf(c2) >= 0) {
                return true;
            }
            return false;
        }
        if (c1 == 'H') {
            if (equA.indexOf(c2) >= 0 || equT.indexOf(c2) > 0 || equC.indexOf(c2) >= 0) {
                return true;
            }
            return false;
        }
        if (c1 == 'D') {
            if (equA.indexOf(c2) >= 0 || equT.indexOf(c2) > 0 || equG.indexOf(c2) >= 0) {
                return true;
            }
            return false;
        }
        if (c1 == 'B') {
            if (equT.indexOf(c2) >= 0 || equG.indexOf(c2) > 0 || equC.indexOf(c2) >= 0) {
                return true;
            }
            return false;
        }
...
function displayHairpin(theHairpinArray, theSequence) {
    ...
    if (theHairpinArrayLength > 0) {
        ...
        for (i = 0; i < theHairpinArrayLength; i++) {
            ...
            // substring wants a value from the start location to 1+the end location
            s1 = theSequence.substring(0, theHairpinArray[i][0]) + theSequence.substring(theHairpinArray[i][0], theHairpinArray[i][1] + 1).fontcolor("red") + ((theHairpinArray[i][1] + 1 >= theHairpinArray[i][2]) ? "-" : "") + theSequence.substring(theHairpinArray[i][1] + 1, theHairpinArray[i][2]) + theSequence.substring(theHairpinArray[i][2], theHairpinArray[i][3] + 1).fontcolor("red") + theSequence.substring(theHairpinArray[i][3] + 1, d)
...
*/
    ...
    if (theAlignedArray.length > 1) {
        ...
        for (n = 0; n < theAlignedArray.length; n++) {
            ...
            if (offset < 0)
                s1 += offsetStr;
            if (debugDimers)
                returnString += "offset=" + offset + "; maxoffset=" + maxoffset + "; [0]=" + theAlignedArray[n][0] + "; [1]=" + theAlignedArray[n][1] + ";<br>";
            // substring wants a value from the start location to 1+the end location
            s1 += theSequence.substring(0, theAlignedArray[n][0]) + theSequence.substring(theAlignedArray[n][0], theAlignedArray[n][2] + 1).fontcolor("red") + theSequence.substring(theAlignedArray[n][2] + 1, d);
            returnString += "5' " + s1 + ((offset > 0) ? offsetStr + maxoffsetStr : maxoffsetStr) + " 3'<BR>";
            s2 += reversedSeq.substring(0, theAlignedArray[n][1]) + reversedSeq.substring(theAlignedArray[n][1], theAlignedArray[n][3] + 1).fontcolor("red") + reversedSeq.substring(theAlignedArray[n][3] + 1, d);
            returnString += "3' " + s2 + ((offset < 0) ? offsetStr + maxoffsetStr : maxoffsetStr) + " 5'<BR>&nbsp;<BR>";
        }
    } else {
        returnString += " None !<BR>";
    ...
    calcHairpin = execjs.compile(oligo_js)
    theHairpinArray = calcHairpin.call('calcHairpin', seq, 4)
    displayHairpin = calcHairpin.call('displayHairpin', theHairpinArray, seq)

    display3enddimer = display3EndDimer(AlignedArray, seq, seq_list, compare_list)
    displayAllD = displayAllDimers(AlignedArray, seq, compare)

    if 'None' in displayHairpin and 'None' in display3enddimer and 'None' in displayAllD:
        return seq
...
def Oligo(seq_list):
    ...
    for seq in seq_list:
        all_none_seq = AllNoneSeq(seq)
        if all_none_seq:
            ret_seq_list.append(all_none_seq)
    return ret_seq_list


# if __name__ == '__main__':
#     seq_list = ['GCCTTAAACAAGTTCCGAGAAAC', 'TCCCATTGGAGAGCTAGAAGCC', 'TGACTTTAGTCAGCGACAGAAGG',
#                 'CCTTTTTAGTCAGCGACAAAAGG']
#
#     none_seq_list = Oligo(seq_list)
#
#     print(none_seq_list)
