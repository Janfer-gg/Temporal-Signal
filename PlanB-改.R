...
ko_seq<-data.frame(seq=character())
if(nrow(KO_region_20)!=0){
  ...
  write.csv(ko_seq,"sequence.csv",row.names=FALSE)
  source_python("crispor_table_download.py")
...
# 画图 ----------------------------------------------------------------------
{
  if (nrow(gRNA) != 0) {
    ...
    img2<-'cut2.png'
    p1_large <-
      p1_large + geom_image(data = ff,aes(x = x, y = y),image=img)+annotate("text",label="g1",x = gRNA[1, ]$end+60,y = 0.56,size=3)
    if(nrow(gRNA)==2){
      fff <-data.frame(x =gRNA[2, ]$start, y = 0.42)
      p1_large<-p1_large+geom_image(data = fff,aes(x = x, y = y),image=img2)+annotate("text",label="g2",x = gRNA[2, ]$end+60,y = 0.44,size=3)
    }
