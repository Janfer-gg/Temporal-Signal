KO_longregion <- function(t_Exon_CDS) {
  if (max(t_Exon_CDS$end) - min(t_Exon_CDS$start) <= 3000) {
    KO_region4 <- t_Exon_CDS[1, ]
    KO_region4$start <- min(t_Exon_CDS$start)
    KO_region4$end <- max(t_Exon_CDS$end)
    Exon_length <- sum(t_Exon_CDS$end-t_Exon_CDS$start+1)
    KO_region4<-cbind(KO_region4,Exon_length)
    return(KO_region4)
  }
    ...
    }
    {
      if (q == 1) {
        ...
        t_CDS_30<-KO_region[q, ]$start +  CDS_length_30 - sum(CDS_length[1:(q - 1)])
      }
    }
    ...
    #   #删除起始密码子所在的外显子
    #   if (Gene_rev) {
    #     if (KO_region3[i, ]$end == t_CDS_start) {
    #       region_del <- append(region_del, i)
    #       Exon_start_t <- KO_region3[i, ]$start
    #       for (j in (i + 1):nrow(KO_region3)) {
    #         if (KO_region3[j, ]$end + 500 > Exon_start_t) {
    #           region_del <- append(region_del, j)
    #           Exon_start_t <- KO_region3[j, ]$start
    #         }
    #       }
    #     }
    #   }
    #   
    #   else{
    #     if (KO_region3[i, ]$start == t_CDS_start) {
    #       region_del <- append(region_del, i)
    #       Exon_end_t <- KO_region2[i, ]$end
    #       for (j in (i + 1):nrow(KO_region3)) {
    #         if (KO_region3[j, ]$start - 500 < Exon_end_t) {
    #           region_del <- append(region_del, j)
    #           Exon_end_t <- KO_region3[j, ]$end
    #         }
    #       }
    #     }
    #   }
    ...
    for (i in 1:nrow(KO_region2)) {
      ...
      Exon_length[i] <-
        t_Exon_region_sort[j,]$Exon_end - t_Exon_region_sort[j,]$Exon_start + 1
      #往左边合并
      ko_start <- t_Exon_region_sort[j,]$Exon_start - 500
      repeat {
        if (j == 1) {
          break
        }
      ...
      #往右边合并
      ko_end <- t_Exon_region_sort[j,]$Exon_end + 500
      repeat {
      ...
      KO_region2[i,]$start <- ko_start + 500
      KO_region2[i,]$end <- ko_end - 500
    }
        ...
        if (nrow(KO_region2) == 1) {
          if (length %% 3 != 0) {
            if (Gene_rev) {
              ...
              if (KO_region4$start == t_Exon_region_sort[1,]$Exon_start) {
              ...
              if (Gene_rev) {
                ...
                if (KO_region4$end - KO_region4$start >= 3000) {
                  KO_region4$Exon_length <-
                    sum(KO_region2[1:j,]$Exon_length)
                  if (KO_region4$start == t_Exon_region_sort[1,]$Exon_start) {
                    KO_region4$start <- t_Exon_CDS[nrow(t_Exon_CDS),]$start
                  }
