...
#创建文件夹
filepath<-"C://Users//41518//Desktop//ISG15"
dir.create(filepath)
library(ggplot2)
library(ggimage)
library(magick)
...
if(nrow(KO_region_20)!=0){
  for(t in 1:nrow(KO_region_20)){
    ...
    for (i in 1:length(gRNA.table[, 1])) {
      ...
      else if (grepl("Not with U6", gRNA.table[i, 2])) {
        gRNA.del <- append(gRNA.del, i)
      }
    }
      ...
      if (Gene_rev) {
        ...
        for (i in 1:length(gRNA.table[, 1])) {
          if (gRNA.table[i,]$strand == "rev") {
            ...
            rev <- matchPattern(pattern = gRNA_rev, subject = gene)
            rev_start <- start(rev)
            rev_end <- end(rev)
            gRNA.table[i, 5] <- rev_start
            gRNA.table[i, 6] <- rev_end
    ...
    #符合条件的gRNA进行切割效率预测
    write.csv(gRNA.table, file = paste0(filepath,"//CCTOP-predictor2.csv"), row.names = FALSE)
    source_python("crispr_get_score.py")
    py$reader_writer(paste0(filepath,"//CCTOP-predictor2.csv"),species)
    gRNA.table <- read.csv(paste0(filepath,"//CCTOP-predictor2.csv"), header = TRUE)
    print(gRNA.table)
    #切割效率得分低于0.60的删除
...
if(nrow(gRNA)!=0){
  ...
  if(nrow(gRNA)>=2){
    ...
    t_Exon_CDS2<-rbind(t_Exon_CDS2,t_Exon_CDS3)
  ...
  if(nrow(gRNA)==3){
    ...
    t_Exon_CDS2<-rbind(t_Exon_CDS2,t_Exon_CDS4)
  }
  
  t_Exon_CDS2<-t_Exon_CDS2[!duplicated(t_Exon_CDS2$Exon),]
  which_ko <- t_Exon_CDS2$Exon
  
  print(which_ko)
}

KO_region_start<-min(gRNA$start)
KO_region_end<-max(gRNA$end)
...
# 画图 ----------------------------------------------------------------------
{
  if (nrow(gRNA) != 0) {
    ...
    # 敲除区域放大图 -----------------------------------------------------------------
    # gRNA间隔太大
    if(max(gRNA$end)-min(gRNA$start)>20000){
      distance <- max(gRNA$end)-min(gRNA$start)
      {
        if(distance<50000){
          p<-0.45
        }
      ...
      if(nrow(t_Exon_CDS2)==2){
        gRNA[which.min(gRNA$start),]$start<-floor(gRNA[which.min(gRNA$start),]$start+p*distance)
        gRNA[which.min(gRNA$start),]$end<-gRNA[which.min(gRNA$start),]$start+19
        
        t_Exon_CDS2[which.min(t_Exon_CDS2$start),]$start<-floor(t_Exon_CDS2[which.min(t_Exon_CDS2$start),]$start+p*distance)
        t_Exon_CDS2[which.min(t_Exon_CDS2$start),]$end<-floor(t_Exon_CDS2[which.min(t_Exon_CDS2$start),]$end+p*distance)
        
        gRNA[which.max(gRNA$start),]$start<-floor(gRNA[which.max(gRNA$start),]$start-p*distance)
        gRNA[which.max(gRNA$start),]$end<-gRNA[which.max(gRNA$start),]$start+19
        
        t_Exon_CDS2[which.max(t_Exon_CDS2$start),]$start<-floor(t_Exon_CDS2[which.max(t_Exon_CDS2$start),]$start-p*distance)
        t_Exon_CDS2[which.max(t_Exon_CDS2$start),]$end<-floor(t_Exon_CDS2[which.max(t_Exon_CDS2$start),]$end-p*distance)
        
        if(t_Exon_region2$Exon_start>max(t_Exon_CDS2$end)){
          t_Exon_region2$Exon_start<-floor(t_Exon_region2$Exon_start-p*distance)
          t_Exon_region2$Exon_end<-floor(t_Exon_region2$Exon_end-p*distance)
        }
        else if(t_Exon_region2$Exon_end<min(t_Exon_CDS2$start)){
          t_Exon_region2$Exon_start<-floor(t_Exon_region2$Exon_start+p*distance)
          t_Exon_region2$Exon_end<-floor(t_Exon_region2$Exon_end+p*distance)
        }
      }
      else if(nrow(t_Exon_CDS2)==3){
        length1<-floor(gRNA[which(gRNA$start==sort(gRNA$start)[2]),]$start-(0.5-p)*distance)-gRNA[which.min(gRNA$start),]$start
        gRNA[which.min(gRNA$start),]$start<-gRNA[which.min(gRNA$start),]$start+length1
        gRNA[which.min(gRNA$start),]$end<-gRNA[which.min(gRNA$start),]$start+19
        
        t_Exon_CDS2[which.min(t_Exon_CDS2$start),]$start<-t_Exon_CDS2[which.min(t_Exon_CDS2$start),]$start+length1
        t_Exon_CDS2[which.min(t_Exon_CDS2$start),]$end<-t_Exon_CDS2[which.min(t_Exon_CDS2$start),]$end+length1
        
        length2<-gRNA[which.max(gRNA$start),]$start-floor(gRNA[which(gRNA$start==sort(gRNA$start)[2]),]$end+(0.5-p)*distance)
        gRNA[which.max(gRNA$start),]$start<-gRNA[which.max(gRNA$start),]$start-length2
        gRNA[which.max(gRNA$start),]$end<-gRNA[which.max(gRNA$start),]$start+19
        
        t_Exon_CDS2[which.max(t_Exon_CDS2$start),]$start<-t_Exon_CDS2[which.max(t_Exon_CDS2$start),]$start-length2
        t_Exon_CDS2[which.max(t_Exon_CDS2$start),]$end<-t_Exon_CDS2[which.max(t_Exon_CDS2$start),]$end-length2
        
        if(t_Exon_region2$Exon_start>max(t_Exon_CDS2$end)){
          t_Exon_region2$Exon_start<-t_Exon_region2$Exon_start-length2
          t_Exon_region2$Exon_end<-t_Exon_region2$Exon_end-length2
        }
        else if(t_Exon_region2$Exon_end<min(t_Exon_CDS2$start)){
          t_Exon_region2$Exon_start<-t_Exon_region2$Exon_start+length1
          t_Exon_region2$Exon_end<-t_Exon_region2$Exon_end+length1
        }
...
if (nrow(gRNA) != 0) {
  ...
  # 所有转录本的展示图 ---------------------------------------------------------------
  p2<-KO_region_image2(Gene,allinfo,KO_region_start,KO_region_end)
