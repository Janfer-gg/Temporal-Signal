#各转录本及其外显子示意图
KO_region_image2 <- function(Gene,allinfo,KO_region_start,KO_region_end) {
  ...
  cut_large<-min(transcript.pos$y)
  {
    if (cut_large >= -2.1) {
      img <-
        image_crop(image_read(paste0(filepath,"//","transcript_region2.png")), "x260")
    }
    else if (cut_large >= -4.2) {
      img <-
        image_crop(image_read(paste0(filepath,"//","transcript_region2.png")), "x420")
    }
    else if (cut_large >= -6.3) {
      img <-
        image_crop(image_read(paste0(filepath,"//","transcript_region2.png")), "x580")
    }
    else if (cut_large >= -8.4) {
      img <-
        image_crop(image_read(paste0(filepath,"//","transcript_region2.png")), "x700")
    }
    else if (cut_large >= -10.5) {
      img <-
        image_crop(image_read(paste0(filepath,"//","transcript_region2.png")), "x840")
    }
      ...
      img <-
        image_crop(image_read(paste0(filepath,"//","transcript_region2.png")), "x1120")
    }
    else if (cut_large >= -18) {
      img <-
        image_crop(image_read(paste0(filepath,"//","transcript_region2.png")), "x1320")
    }
    else{
      img <- image_read(paste0(filepath,"//","transcript_region2.png"))
    }
  }
  return(img)
}
