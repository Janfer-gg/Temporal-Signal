...
image1<-function(output,Exon_region){
  primer.table<-output
  {
    #反向基因
    if(Gene_rev){
  ...
  for(i in 1:nrow(Exon_region)){
    p2 <- p2 + annotate("rect",xmin = Exon_region[i,]$start,xmax = Exon_region[i,]$end,ymin = 0.96,ymax = 1.04,fill = "#D01027")+
      annotate("text",label=Exon_region[i,]$Exon,x=(Exon_region[i,]$start+Exon_region[i,]$end)/2,y=0.94,size=4)
  }
  return(p2)
}
...
#只有外侧靶位点
image2<-function(output,Exon_region){
  primer.table<-output
  {
    #反向基因
    if(Gene_rev){
...
#有一个内侧靶位点
image3<-function(output,Exon_region){
  primer.table<-output
  {
    #反向基因
    if(Gene_rev){
