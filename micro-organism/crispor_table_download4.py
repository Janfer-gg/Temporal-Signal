...
if __name__ == '__main__':
    with open('C:/Users/41518/Desktop/微生物/Saccharomyces/Sac.csv', 'r', encoding='utf8')as aa:
        reader = csv.reader(aa)
        gene_table=[]
        for line in reader:
            gene_table.append(line)
        for i in range(1527,len(gene_table)):
            gRNA1 = gene_table[i][10]
            gRNA2 = gene_table[i][14]

            if gRNA1 !='NA':
                Lindel1 = run(gRNA1, gene_table[i][0], 'C://Users//41518//Desktop//')
                gene_table[i][33]= Lindel1
                del Lindel1
            else:
                gene_table[i][33]= 'NA'

            if gRNA2 !='NA':
                Lindel2 = run(gRNA2, gene_table[i][0], 'C://Users//41518//Desktop//')
                gene_table[i][34]= Lindel2
                del Lindel2
            else:
                gene_table[i][34] = 'NA'

            with open('C:/Users/41518/Desktop/微生物/Saccharomyces/Sac-lindel.csv','a',encoding='utf8',newline='') as f:
                writer = csv.writer(f)
                writer.writerow(gene_table[i])
                f.close()
            print(i,gene_table[i][2])
