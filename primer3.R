...

{
  ...
  if (Gene_rev) {
    ...
    # Blast -------------------------------------------------------------------
    source_python("C://Users//41518//Desktop//work//Ubigene//primertool.py") 
    if (nrow(PCR2.table) != 0) {
      py$run_primertool(
        "C:\\Users\\41518\\Desktop\\work\\Ubigene\\PCR4.csv",
        "C:\\Users\\41518\\Desktop\\work\\Ubigene\\PCR3.csv",
        "C:\\Users\\41518\\Desktop\\work\\Ubigene\\primer_result1.csv",
        species
      )
      primer.table <-
        read.csv("primer_result1.csv", header = TRUE)
      if(nrow(primer.table) != 0){
        #计算靶位点距离(反向基因),内侧靶位点与上游配对
        primer3<-primer.table[1,2]
        if (min(gRNA$start) < 750 | max(gRNA$end) + 750 > nchar(Gene)) {
          primer3.pos<-matchPattern(complement(DNAString(primer3)),subject = Gene2)
          
        }
        else{
          primer3.pos<-matchPattern(complement(DNAString(primer3)),subject = Gene)
        }
        WT2.distance<-max(end(primer1.pos),end(primer3.pos))-min(start(primer1.pos),start(primer3.pos))+1
      }
    }
    ...
    if (nrow(primer.table) == 0) {
      py$run_primertool(
        ...
        "C:\\Users\\41518\\Desktop\\work\\Ubigene\\primer_result2.csv",
        species
      )
      primer.table <-
      ...
      if (nrow(primer.table) != 0) {
        #计算靶位点距离(反向基因),内侧靶位点与下游配对
        primer4 <- primer.table[1, 1]
        if (min(gRNA$start) < 750 | max(gRNA$end) + 750 > nchar(Gene)) {
          primer4.pos<-matchPattern(reverse(DNAString(primer4)),subject = Gene2)
          
        }
        else{
          primer4.pos<-matchPattern(reverse(DNAString(primer4)),subject = Gene)
        }
        WT2.distance<-max(end(primer2.pos),end(primer4.pos))-min(start(primer2.pos),start(primer4.pos))+1
      }
    }
    print(primer.table)
  }
  ...
  # 正向基因 --------------------------------------------------------------------
    ...
    if (nrow(PCR2.table) != 0) {
      ...
      if(nrow(primer.table) != 0){
        ...
        primer3<-primer.table[1,2]
        if (min(gRNA$start) < 750 | max(gRNA$end) + 750 > nchar(Gene)) {
          primer3.pos<-matchPattern(reverseComplement(DNAString(primer3)),subject = Gene2)
          
        }
        else{
          primer3.pos<-matchPattern(reverseComplement(DNAString(primer3)),subject = Gene)
        }
        WT2.distance<-max(end(primer1.pos),end(primer3.pos))-min(start(primer1.pos),start(primer3.pos))+1
      }
    ...
    if (nrow(primer.table) == 0) {
      ...
      if(nrow(primer.table) != 0){
        ...
        primer4<-primer.table[1,1]
        if (min(gRNA$start) < 750 | max(gRNA$end) + 750 > nchar(Gene)) {
          primer4.pos<-matchPattern(DNAString(primer4),subject = Gene2)
        }
        else{
          primer4.pos<-matchPattern(DNAString(primer4),subject = Gene)
        }
        WT2.distance<-max(end(primer2.pos),end(primer4.pos))-min(start(primer2.pos),start(primer4.pos))+1
      }
    }
