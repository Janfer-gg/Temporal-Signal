...
def single_guide_genotype(guide,wt_data,type,del_before=0,del_after=0,insert=0):
    ...
    if type == 'delete':
        ...
        for i in delete_bases:
            ...
            dac += 1
        summary = {"type": "single", "total": -len(delete_bases), "label": label}
        return {'primary_seq': wt_primary_seq, 'sequence': (''.join(wt_primary_seq)).replace('-', ''), 'trace_data': wt_trace_data, 'cutsite': cutsite, 'change': -len(delete_bases), 'type': '{}[{}]'.format(-(len(delete_bases)), label), 'summary': summary}
        ...
        summary = {"type": "single", "total": '+' + str(insert), "label": label}
        return {'primary_seq': wt_primary_seq, 'sequence': (''.join(wt_primary_seq)).replace('-', ''), 'trace_data': wt_trace_data, 'cutsite': cutsite, 'change': insert, 'type': '{}[{}]'.format(+(insert), label), 'summary': summary}
        # print(wt_primary_seq,wt_trace_data)

def mutiple_guide_genotype(guide1,guide2,wt_data,type,cut1_before=0,cut1_after=0,cut2_before=0,cut2_after=0,cut1_insert=0,cut2_insert=0,dropout=False):
    ...
    if type == 'delete':
        ...
        summary = {'tpye': 'mutiple', 'total': -len(delete_bases), 'details': {'label1': label1, 'indel1':-len(cut1_bases), 'label2': label2, 'indel2':-len(cut2_bases)}}
        return {'primary_seq': wt_primary_seq, 'sequence':(''.join(wt_primary_seq)).replace('-',''), 'trace_data': wt_trace_data, 'cutsite': (cutsite1,cutsite2),
                'change': -len(delete_bases), 'type': '-{} -{}[{}],-{}[{}]'.format(len(delete_bases), len(cut1_bases), label1, len(cut2_bases), label2), 'summary': summary}
    ...
    if type == 'insert':
        ...
        summary = {'type': 'mutiple', 'total': base_change, 'details': {'label1': label1, 'indel1': '+'+str(cut1_insert), 'label2': label2, 'indel2': '+'+str(cut2_insert)}}

        return {'primary_seq': wt_primary_seq, 'sequence': (''.join(wt_primary_seq)).replace('-', ''), 'trace_data': wt_trace_data,
                'cutsite': (cutsite1 + cut1_insert, cutsite2 + cut1_insert + cut2_insert), 'change': base_change, 'type': '{} +{}[{}] +{}[{}]'.format(base_change, cut1_insert, label1, cut2_insert, label2),
                'summary': summary}
    ...
    with open(filename, 'w') as f:
        for genotype in genotypes:
            if isinstance(genotype['cutsite'], int):
                ...
                primary_seq = copy(genotype['primary_seq'])
                primary_seq.insert(cutsite, '|')
                start = cutsite - indel_max_size - 5
                end = cutsite + indel_max_size +20
                f.write(''.join(primary_seq[start:end])+'  '+genotype['type']+'\n')

            elif isinstance(genotype['cutsite'], tuple):
                cutsite1 = genotype['cutsite'][0]
                cutsite2 = genotype['cutsite'][1]
                primary_seq = copy(genotype['primary_seq'])
                primary_seq.insert(cutsite2, '|')
                primary_seq.insert(cutsite1, '|')
