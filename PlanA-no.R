...
#创建文件夹
filepath<-"C://Users//41518//Desktop//0925测试//FLCN"
dir.create(filepath)
library(ggplot2)
library(ggimage)
library(magick)
...
source("Get_result1-no.R")
source("Get_result2-no.R")
      ...
      for (j in 1:nrow(KO_region_500)) {
        if (all(KO_region[t, ] %in% KO_region_500[j, ])) {
          ...
          for (i in 1:length(gRNA.table1[, 1])) {
            ...
            else if (grepl("Not with U6", gRNA.table1[i, 2])) {
              gRNA.del <- append(gRNA.del, i)
            }
          }
          if(length(gRNA.del)!=0){
          ...
          for (i in 1:length(gRNA.table2[, 1])) {
            ...
            else if (grepl("Not with U6", gRNA.table2[i, 2])) {
              gRNA.del <- append(gRNA.del, i)
            }
          }
          if(length(gRNA.del)!=0){
      ...
      for (a in 1:nrow(KO_region3)) {
        ...
        #table1是外显子上游的gRNA,table2是外显子下游的gRNA
        {
          if (nrow(gRNA.table1) == 0 | nrow(gRNA.table2) == 0) {
            ...
            gRNA <- Get_result1(gRNA.table, KO_region3[a,])       
          }
        }
        #如果没有相差0.05分以内的gRNA
        if (class(gRNA) == "NULL") {
