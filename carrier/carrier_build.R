...
build_carrier1<-function(access,carname,number,gRNA1,scaffold,account){
  library(openxlsx)
  library(Biostrings)
  
  Mydata<-read.xlsx("C://Users//41518//Desktop//work//ubigene/carrier/carrier.xlsx",sheet=1,fillMergedCells=TRUE)
  Mydata2<-data.frame()
  for(i in 1:account){
    ...
    names(Mydata2)<-c("Э���","��������","�ڲ����","gRNA����","�Ǽ���Ϣ","Oligo-F","Oligo-F","Oligo-R","Oligo-R","�������","��������","ø�з���")
  }
  ...
  saveWorkbook(wb, "C://Users//41518//Desktop//carrier.xlsx", overwrite = TRUE)
  return(TRUE)
}

build_carrier2<-function(access,carname,number,gRNA1,gRNA2,scaffold,account){
  library(openxlsx)
  library(Biostrings)
  TM<-function(seq){
  ...
  Mydata<-read.xlsx("C://Users//41518//Desktop//work/ubigene/carrier//carrier.xlsx",sheet=2,fillMergedCells=TRUE)
  Mydata2<-data.frame()
  for(i in 1:account){
    Mydata3<-Mydata[which(Mydata[,2]==scaffold[i]),]
    if(nrow(Mydata3)==0){
    ...
    Mydata2[i*2,9] <- paste0("TGTGGAAAGGACGAAACACC","g",gRNA1[[i]])
    ...
    Mydata2[i*2-1,12] <- "�ϳɻ���scaffold-U6"
    ...
    names(Mydata2)<-c("Э���","��������","�ڲ����","gRNA1����","gRNA2����","�Ǽ���Ϣ","�����ִ�","Oligo-F","Oligo-F","Oligo-R","Oligo-R","PCRģ��","Ƭ������(��С)","�������","��������","ø�з���")
  }
...
build_carrier3<-function(access,carname,number,gRNA1,scaffold,account){
  library(openxlsx)
  library(Biostrings)
  
  Mydata<-read.xlsx("C://Users//41518//Desktop//work//ubigene/carrier/carrier.xlsx",sheet=3,fillMergedCells=TRUE)
  Mydata2<-data.frame()
  for(i in 1:account){
    Mydata3<-Mydata[which(Mydata[,2]==scaffold[i]),]
    if(nrow(Mydata3)==0){
    ...
    Mydata2[i,5] <- scaffold[i]
    Mydata2[i,6] <- paste0(number[i],"-F")
    g <- as.character(reverseComplement(DNAString(as.character(gRNA1[i]))))
    ...
    Mydata2[i,11] <-Mydata3[,8]
    Mydata2[i,12] <-Mydata3[,9]
    names(Mydata2)<-c("Э���","��������","�ڲ����","shRNA����","�Ǽ���Ϣ","Oligo-F","Oligo-F","Oligo-R","Oligo-R","�������","��������","ø�з���")
  }
...
# aa<-build_carrier(type = 2,scaffoldname = "YKO-RP005",carname<-"YKO-RP005-Cd44[gRNA1]",gRNA1 ="AATGTAACCTGCCGCTACGC",gRNA2 ="CAACTTCATTTGGTCCATGG" ,number = "CK20-115-a")
# 
#access<-c("UBISCI201015JC1","UBISCI201015JC1","UBISCI201015JC1")
#carname<-c("YSH-LV001-hITGA5[shRNA1]","YSH-LV001-hITGA5[shRNA2]","YSH-LV001-hITGA5[shRNA3]")
#number<-c("CE20-060a","CE20-060b","CE20-060c")
#gRNA1<-c("GGTGGACCAGGAAGCTATTTC","CTCAGGCCAGCCCTACATTAT","CTCCTATATGTGACCAGAGTT")
## gRNA2<-c("AACGTTTGTCCTTGCTCCTG","GGAGTAGTTGGCAGATCCAC")
#scaffold<-c("YSH-LV001","YSH-LV001","YSH-LV001")
#account<-3
#build_carrier3(access,carname,number,gRNA1,scaffold,account)
TM<-function(seq){
    GC_count<-length(gregexpr("[GCgc]",seq)[[1]])
    tm<-100.5 + (41*(GC_count)/nchar(seq)) - (820/nchar(seq)) + 16.6*log(0.05,10)
    print(tm)
}
TM("TAGCTACAGTGAAATCTCGA")